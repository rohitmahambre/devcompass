    ...
    with gr.Blocks(
        ...
        gr.HTML(
            ...
            head=(
                ...
                "<style>"
                "  /* Fix all Mermaid diagram rendering: white background, dark text/lines, light blue nodes */"
                "  .mermaid,"
                "  .mermaid svg,"
                "  .prose svg,"
                "  svg[id^='mermaid-'] {"
                ...
                "    border-radius: 8px;"
                "  }"
                "  /* Force dark colors on labels/text/spans inside Mermaid SVGs */"
                "  .mermaid svg text, .mermaid svg span, .mermaid svg div, .mermaid svg foreignObject *,"
                "  .prose svg text, .prose svg span, .prose svg div, .prose svg foreignObject *,"
                "  svg[id^='mermaid-'] text, svg[id^='mermaid-'] span, svg[id^='mermaid-'] div, svg[id^='mermaid-'] foreignObject * {"
                "    color: #1a1a1a !important;"
                "    fill: #1a1a1a !important;"
                "  }"
                "  /* Force dark outline on all path connections and lines */"
                "  .mermaid svg .edgePath path,"
                "  .prose svg .edgePath path,"
                "  svg[id^='mermaid-'] .edgePath path {"
                "    stroke: #2c3e50 !important;"
                "    stroke-width: 1.5px !important;"
                "  }"
                "  /* Force node styling: light blue background, blue borders */"
                "  .mermaid svg .node rect, .mermaid svg .node circle, .mermaid svg .node polygon,"
                "  .prose svg .node rect, .prose svg .node circle, .prose svg .node polygon,"
                "  svg[id^='mermaid-'] .node rect, svg[id^='mermaid-'] .node circle, svg[id^='mermaid-'] .node polygon {"
                ...
                "    var els = document.querySelectorAll('.mermaid[data-src]');"
                "    if (els.length === 0) return;"
                "    var needsRun = false;"
                "    els.forEach(function(el) {"
                "      var src = el.getAttribute('data-src');"
                "      if (el.getAttribute('data-rendered-src') === src) return;"
                "      try {"
                "        el.textContent = atob(src);"
                "        el.setAttribute('data-rendered-src', src);"
                "        el.removeAttribute('data-processed');"
                "        el.innerHTML = atob(src);"
                "        needsRun = true;"
                "      } catch(e) {"
                ...
                "            primaryBorderColor: '#2980b9', lineColor: '#2c3e50', background: '#ffffff' } });"
                ...
                "    obs.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['data-src'] });"
                "    renderPending();"
                "  });"
                "  /* Check periodically as fallback to ensure rendering */"
